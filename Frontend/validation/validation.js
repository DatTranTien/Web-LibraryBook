...
exports.validateRegistration =(body)=>{
...
;
    ...
    let errors = {}
    if (body?.firstName?.trim().length < 2 || body.firstName == undefined || 
    !reunicode.test(body?.firstName?.trim())
    ) {
        console.log("body.firstName == undefined===>",body.firstName == undefined)
        console.log("body?.firstName?.trim().length < 2",body?.firstName?.trim().length )
        console.log("check fisrt name: ",!/^[A-Za-z]+$/.test(body?.firstName?.trim()))
        errors.firstNameMsg = 'First name is required'
    }

    if (body?.lastName?.trim().length < 2 || body.lastName == undefined ||
    !reunicode.test(body?.lastName?.trim())
    ) {
        errors.lastNameMsg = 'Last name is required'
    }

    if (body?.address?.trim().length < 3 || body.address == undefined
    ) {
        errors.addressNameMsg = 'Adress must be 3 characters or more'
    }

    if (body.city?.trim().length < 2 || body.city == undefined 
    ) {
        errors.cityMsg = 'City is required'
    }

    if (body?.state?.trim().length < 2 || body.state == undefined
    ) {
        errors.stateMsg = 'State is required'
    }

    if (body.zipcode?.trim().length < 2 || body.zipcode == undefined ||
    /^\d$/.test(body?.zipcode?.trim())
    ) {
        errors.zipcode = 'zipcode is required'
    }

    if (body?.emailAddress?.trim().length < 2 || body.emailAddress == undefined ||
    !re.test(body?.emailAddress?.trim())
    ) {
...
!/(?=(.*[0-9]))(?=.*[\!@#$%^&*()\\[\]{}\-_+=~`|:;"'<>,./?])(?=.*[a-z])(?=(.*[A-Z]))(?=(.*)).{8,}/ .test(body?.password?.trim())
    ) {
        errors.passwordMsg = 'password is not valid'
    }

    if (body.confirmPassword == undefined ||
    !// Bao gồm cả chữ hoa, chữ thường, số, ký tự đặc biệt và ít nhất 8 kỹ tự
!/(?=(.*[0-9]))(?=.*[\!@#$%^&*()\\[\]{}\-_+=~`|:;"'<>,./?])(?=.*[a-z])(?=(.*[A-Z]))(?=(.*)).{8,}/ .test(body?.confirmPassword?.trim())
    ) {
        errors.confirmPasswordMsg = 'confirmPassword is not valid'
    }

    if (body?.password?.trim() !== body?.confirmPassword?.trim() || body.password == undefined ||body.confirmPassword == undefined  ) {
        errors.confirmPasswordMsg = 'Password do not match'
    }

    return errors
}

exports.validationLogin=(body)=>{
    let errors = {}
    if (body?.emailAddress?.trim().length < 2 || body.emailAddress == undefined ||
    !re.test(body?.emailAddress?.trim())
    ) {
